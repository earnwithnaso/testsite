        ...
        <div class="bg-white rounded-5xl shadow-medium p-10">
            ...
            <form action="{{ route('admin.lessons.update', $lesson) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $lesson->title) }}" class="w-full h-14 px-6 rounded-full border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary" required>
                ...
                <div class="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="video_duration" value="{{ old('video_duration', $lesson->video_duration) }}" class="w-full h-14 px-6 rounded-full border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary">
                    ...
                    <!-- Position -->
                     <div>
                        ...
                        <input type="number" name="position" value="{{ old('position', $lesson->position) }}" class="w-full h-14 px-6 rounded-full border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" class="w-full p-6 rounded-4xl border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary resize-none">{{ old('description', $lesson->description) }}</textarea>
                ...
                <div class="flex gap-8 ml-4">
                     <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_published" value="1" class="sr-only peer" {{ $lesson->is_published ? 'checked' : '' }}>
                ...
                <div class="flex items-center justify-end gap-4 pt-8">
                    <a href="{{ route('admin.courses.lessons.index', $lesson->course_id) }}" class="px-8 py-4 font-bold text-secondary hover:text-primary transition-colors">Cancel</a>
