<x-admin-layout>
    ...
    <div class="mb-8">
        ...
        <h2 class="text-2xl font-black text-primary tracking-tight">Edit Quiz: {{ $quiz->title }}</h2>
    ...
    <div class="bg-white rounded-5xl shadow-soft p-10 max-w-2xl">
        <form action="{{ route('admin.quizzes.update', $quiz) }}" method="POST" class="space-y-8">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" value="{{ old('title', $quiz->title) }}" class="w-full px-6 py-4 rounded-2xl bg-soft-grey/30 border-2 border-transparent focus:border-primary focus:bg-white outline-none transition-all font-bold text-primary" required>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-2">
                    ...
                    <select name="lesson_id" class="w-full px-6 py-4 rounded-2xl bg-soft-grey/30 border-2 border-transparent focus:border-primary focus:bg-white outline-none transition-all font-bold text-primary">
                        ...
                        @foreach ($lessons as $lesson)
                            <option value="{{ $lesson->id }}" {{ $quiz->lesson_id == $lesson->id ? 'selected' : '' }}>{{ $lesson->title }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="passing_percentage" value="{{ old('passing_percentage', $quiz->passing_percentage) }}" min="0" max="100" class="w-full px-6 py-4 rounded-2xl bg-soft-grey/30 border-2 border-transparent focus:border-primary focus:bg-white outline-none transition-all font-bold text-primary" required>
                </div>
            </div>

            <div class="space-y-2">
                <label class="block text-sm font-black text-primary uppercase tracking-widest">Time Limit (Minutes - Optional)</label>
                <input type="number" name="time_limit" value="{{ old('time_limit', $quiz->time_limit) }}" placeholder="Leave empty for no limit" class="w-full px-6 py-4 rounded-2xl bg-soft-grey/30 border-2 border-transparent focus:border-primary focus:bg-white outline-none transition-all font-bold text-primary">
            </div>

            <div class="space-y-2">
                <label class="block text-sm font-black text-primary uppercase tracking-widest">Description</label>
                <textarea name="description" rows="4" class="w-full px-6 py-4 rounded-2xl bg-soft-grey/30 border-2 border-transparent focus:border-primary focus:bg-white outline-none transition-all font-bold text-primary">{{ old('description', $quiz->description) }}</textarea>
            </div>
