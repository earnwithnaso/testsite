<x-admin-layout>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-5xl shadow-medium p-10">
            <h3 class="text-xl font-bold mb-8 text-primary">New CMS Page</h3>
            ...
            <form action="{{ route('admin.pages.store') }}" method="POST" class="space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" class="w-full h-14 px-6 rounded-full border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary" required>
                ...
                <div>
                    ...
                    <textarea name="content" rows="15" class="w-full p-6 rounded-4xl border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary resize-none font-mono text-sm" placeholder="<p>Page content goes here...</p>">{{ old('content') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="text" name="meta_title" value="{{ old('meta_title') }}" class="w-full h-14 px-6 rounded-full border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-secondary mb-2 ml-4">Meta Description (SEO)</label>
                        <textarea name="meta_description" rows="2" class="w-full p-4 rounded-4xl border-2 border-border-grey focus:border-primary focus:ring-0 transition-colors bg-white text-primary resize-none">{{ old('meta_description') }}</textarea>
                    </div>
                </div>
